...
;;; If we allow duplicates, we can modify adjoin-set to add the
;;; element without checking if the element already exists.
(define (adjoin-set x set)
  (cons x set))

;;; There isn't any way to improve this method
(define (element-of-set? x set)
  (cond ((null? set) false)
        ((equal? x (car set)) true)
        (else (element-of-set? x (cdr set)))))

;;; With the change, the union-set operation required number of steps
;;; grows as O(n)
